...
// ── 前端静态服务 (SPA 回退 + /api /ws 代理, 顺序探测端口) ────────────────────
// 重要: 生产构建的前端用相对路径请求 /api/* 与 /ws (vite dev 的 proxy
// 只在开发模式生效), 因此静态服务必须把 /api 与 /ws 代理到引擎,
// 否则所有业务请求 405/404 —— 这是"界面能开但功能全挂"的根因。
function startWebServer() {
  return new Promise((resolve) => {
    const serve = (req, res) => {
      ...
      // /api 与 /ws → 代理到引擎 (8765)
      if (pathname.startsWith('/api/') || pathname === '/api' || pathname.startsWith('/ws')) {
    ...
    const onUpgrade = (req, socket, head) => {
      ...
      const proxySocket = net.connect(ENGINE_PORT, '127.0.0.1', () => {
    ...
    const tryListen = () => {
      ...
      server.listen(port, '127.0.0.1', () => {
        ...
        state.webPort = port
        log(`前端静态服务: http://localhost:${port} (含 /api /ws 代理)`)
        resolve(port)
      })
    }
    tryListen()
...
// ── 单实例锁 (仅独立 CLI 模式; pidFile 参数化供测试) ────────────────────────
function acquireLock(pidFile = PID_FILE) {
